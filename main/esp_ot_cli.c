...
#define TAG "ot_esp_cli"  // Tag pour les messages de log ESP-IDF
#define LED_GPIO 8  // GPIO connecté à la LED RGB WS2812 intégrée

// Configuration UART pour la communication série (utilisée par le leader)
#define UART_NUM        UART_NUM_1  // Numéro du port UART
#define UART_TX_PIN     5           // Pin de transmission UART
#define UART_RX_PIN     4           // Pin de réception UART
#define UART_BUF_SIZE   1024        // Taille du buffer UART
#define CONTROL_PIN     7           // GPIO pour contrôler une sortie (LED ou relais)

// Variables globales pour la communication UDP
static otUdpSocket sUdpSocket;      // Socket UDP pour envoyer des données à l'enfant
static otIp6Address sChildAddr;     // Adresse IPv6 de l'appareil enfant
static bool sChildAddrSet = false;  // Indicateur si l'adresse de l'enfant est configurée

// Variables pour la réception UDP sur l'appareil enfant
static otUdpSocket sReceiveSocket;  // Socket UDP pour recevoir des commandes
static uint8_t sCurrentLedColor = 0x42;  // Couleur actuelle de la LED (0x42='B'=Bleu, 0x47='G'=Vert)

/**
 * Fonction utilitaire pour vérifier les données UART et contrôler le GPIO 7
 * Cette fonction est appelée depuis la tâche UART du leader pour traiter les données reçues
 */
void check_uart_and_control_pin(uint8_t *data, int len)
{
    if (len > 0) {
        // Vérifie si les données reçues sont 0x00 (commande pour allumer)
        if (data[0] == 0x00) {
            gpio_set_level(CONTROL_PIN, 1);  // Allume le GPIO 7
            ESP_LOGI(TAG, "UART received 0x00 - GPIO %d turned ON", CONTROL_PIN);
        } else {
            gpio_set_level(CONTROL_PIN, 0);  // Éteint le GPIO 7 pour les autres valeurs
            ESP_LOGI(TAG, "UART received 0x%02X - GPIO %d turned OFF", data[0], CONTROL_PIN);
        }
    }
}

#define UDP_PORT        12345   // Port UDP pour la communication
#define CHILD_TIMEOUT_S 60      // Timeout pour les enfants (en secondes)
#define SEND_PERIOD_MS  5000    // Période d'envoi des commandes (en millisecondes)

// Sockets UDP pour envoi et réception
static otUdpSocket sUdpSocket;      // Socket pour envoyer aux enfants
static otUdpSocket sReceiveSocket;  // Socket pour recevoir des commandes

// Indicateurs d'état des sockets
static bool sUdpSocketOpen = false;      // Socket d'envoi ouvert
static bool sReceiveSocketOpen = false;  // Socket de réception ouvert

// Variables pour la gestion des enfants
static otIp6Address sChildAddr;      // Adresse IPv6 de l'enfant
static bool sChildAddrSet = false;   // Adresse enfant configurée
static bool sLedCommandReceived = false;  // Commande LED reçue
static uint8_t sCurrentLedColor = 0x42;   // Couleur LED actuelle ('B' = Bleu)

/**
 * Fonction utilitaire pour traiter les données UART et contrôler GPIO 7
 * Utilisée par la tâche UART du leader pour réagir aux données série
 */
static void check_uart_and_control_pin(const uint8_t *data, int len)
{
    ...
    if (data[0] == 0x00) {
        gpio_set_level(CONTROL_PIN, 1);  // Allume GPIO 7
        ESP_LOGI(TAG, "UART received 0x00 - GPIO %d turned ON", CONTROL_PIN);
    } else {
        gpio_set_level(CONTROL_PIN, 0);  // Éteint GPIO 7
        ESP_LOGI(TAG, "UART received 0x%02X - GPIO %d turned OFF", data[0], CONTROL_PIN);
    }
}

/**
 * Configure l'adresse IPv6 de l'enfant pour la communication
 */
...
/**
 * @brief Initialise le socket UDP pour l'envoi et la réception de messages (version avec verrouillage)
 *
 * Cette fonction configure un socket UDP sur le port spécifié (UDP_PORT) pour permettre
 * la communication entre les appareils parent et enfant dans le réseau Thread.
 * Le socket est lié à l'interface Thread interne pour une communication réseau fiable.
 * Cette version inclut une vérification pour éviter les initialisations multiples.
 *
 * @param instance Instance OpenThread utilisée pour la configuration réseau
...
/**
 * @brief Gestionnaire de réception des messages UDP
 *
 * Cette fonction callback est appelée automatiquement lorsqu'un message UDP est reçu
 * sur le socket configuré. Elle traite les données reçues et interprète les commandes
 * pour contrôler la couleur de la LED RGB.
 *
 * Commandes supportées:
 * - 0x42: Change la couleur de la LED en bleu
 * - 0x47: Change la couleur de la LED en vert
 * - Autres: Commandes inconnues (ignorées avec un avertissement)
 *
 * @param aContext Contexte utilisateur (non utilisé dans cette implémentation)
 * @param aMessage Message UDP reçu contenant les données de commande
 * @param aMessageInfo Informations sur le message (adresse source, port, etc.)
 */
...
/**
 * @brief Initialise le socket UDP de réception avec gestionnaire de callback
 *
 * Cette fonction configure un socket UDP dédié à la réception des messages entrants.
 * Contrairement au socket d'envoi, celui-ci enregistre un callback (handle_udp_receive)
 * qui sera appelé automatiquement lorsqu'un message UDP est reçu.
 *
 * @param instance Instance OpenThread utilisée pour la configuration réseau
 * @return true si l'initialisation réussit, false en cas d'erreur
 */
